...
class AnswerRecognitionAndScoring:
    def __init__(self):
        self.answer_key_manager = AnswerKeyManager()

    def calculate_scores(self, recognized_answers, part_d_digits, marking_digits):
        """
        Calculate the final score based on recognized answers, Part D, and Marking section.

        :param recognized_answers: List of recognized answers for 15 questions.
        :param part_d_digits: List of digits from Part D section.
        :param marking_digits: List of digits from Marking section.
        :return: A dictionary containing detailed score breakdown and final score.
        """
        # Load the answer key
        answer_key = self.answer_key_manager.load_answer_key()

        # Validate answer key
        if not answer_key:
            raise ValueError("Answer key is missing or invalid.")

        # Calculate score from standard questions
        question_scores = 0
        for entry in answer_key:
            ...
            if recognized_answers.get(question_number) == correct_answer:
                question_scores += points

        # Calculate sum of digits from Part D and Marking sections
        part_d_sum = sum(part_d_digits)
        marking_sum = sum(marking_digits)

        # Calculate final score
        final_score = question_scores + part_d_sum + marking_sum
        ...
        for entry in answer_key:
            ...
            mark_allocated = points if recognized_answer == correct_answer else 0

            details[question_number] = {
                "recognized_answer": recognized_answer,
                "correct_answer": correct_answer,
                "mark_allocated": mark_allocated
            }
